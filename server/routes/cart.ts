...
const authUser: RequestHandler = (req, res, next) => {
  ...
  try {
    ...
    if (decoded.type !== "user") {
      return res
        .status(403)
        .json({ success: false, message: "Not authorized as user" });
    }
...
// Add to Cart
router.post("/add", authUser, validateRequest(cartItemSchema), (async (
  req,
  res,
) => {
  try {
...
router.put("/:cartId", authUser, validateRequest(updateCartItemSchema), (async (
  req,
  res,
) => {
  try {
...
// Remove from Cart
router.delete("/:cartId", authUser, (async (req, res) => {
  try {
    const { cartId } = req.params;
    const userId = (req as any).userId;
...
router.post("/calculate-total", authUser, (async (req, res) => {
  try {
