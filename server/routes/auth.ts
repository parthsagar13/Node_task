  ...
  (async (req, res) => {
    try {
      ...
      try {
        ...
        const token = jwt.sign({ userId, email, type: 'user' }, JWT_SECRET, {
          expiresIn: '7d',
        });
  ...
  (async (req, res) => {
    try {
      ...
      try {
        ...
        const token = jwt.sign({ userId: user.id, email: user.email, type: 'user' }, JWT_SECRET, {
          expiresIn: '7d',
        });
...
router.post(
  '/seller/login',
  validateRequest(sellerLoginSchema),
  (async (req, res) => {
    try {
      const { email, password } = req.body;
      const connection = await pool.getConnection();
      ...
      try {
        ...
        const token = jwt.sign(
          { sellerId: seller.id, email: seller.email, type: 'seller' },
          JWT_SECRET,
          { expiresIn: '7d' }
        );
